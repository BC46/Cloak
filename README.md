...
In the in-game options menu, you can set a custom key for the `Toggle Cloak` control (Ctrl + C by default). Using this key, you can cloak and uncloak when in space, provided that the ship you are flying has a cloaking device equipped and that you are in Single Player. Moreover, the available options can be changed in the `cloak.ini` file.
